...
\begin{table}[t]
\centering
\caption{Number of successful runs out of 100 for each setting, where ``Tourney'' is size 7 tournament selection and ``IFS'' is implicit fitness sharing with size 7 tournaments. For each problem, bold indicates significant improvement over the other two at $p < 0.05$ based on a pairwise chi-square test with Holm correction\protect\footnotemark, or a pairwise Fisher's exact test with Holm correction if any number of successes is below 5.}
\label{tableResults}
\begin{tabular}{|l|rrr|}
\hline
...
\footnotetext{(MAKE SURE THIS IS ON THE RIGHT PAGE) In particular, we used R's \texttt{pairwise.prop.test} and \texttt{pairwise.fisher.test}.}
...
Smallest,
Super Anagrams).
These 10 problems show an important area for future study: how to evolve programs that generalize to unseen data for general program synthesis problems.
...
More importantly, paragraph(s) about what the data tells us about the benchmark problems:
- good at differentiating between different techniques
- range of difficulties, from some solved very frequently to others not solved at all

(talk here about things:
which problems do we not solve at all?
which problems solvable by lexicase but not others?
which problems lend themselves to solutions that generalize / don't generalize?)

minimum size of a solution found by GP
